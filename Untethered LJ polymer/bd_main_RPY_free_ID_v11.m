function bd_main_RPY_free_ID_v11(name,np,u1,flow,rep)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
rxt    = 0.002*n_step;       %steps between mc moves
rmx    = zeros(np,np);
...
rep_num = num2str(rep);
intial_conf = strcat('init_',num2str(400),'_',rep_num);
...
%% Start!
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Relaxation from Full Length 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf('\n Equilibration Step 1 \n')
% calculate initial hydrodynamic tensor
D = bd_RPY_hyd_v1(np,n3,a,L,mu,x,y,z);
D = D/(6*pi*mu*a);
[B,~] = chol(kT*D,'lower');
%% Equilibrate with no Flow 
Vx = 0; Vy = 0; Vz = 0;      
for i = 1:n_equi
     [~, TFX, TFY, TFZ, ~] = bd_force_free_ID(np,n3,u,a,K,L,x,y,z,mu); % caclculate forces
          if mod(i,period) == 0 %reupdate hydrodynamic tensor 
             D = bd_RPY_hyd_v1(np,n3,a,L,mu,x,y,z);
             D = D/(6*pi*mu*a);
             [B,~] = chol(kT*D,'lower');
          end
       
      [x, y, z] = bd_move_rpy_v2(n3, x, y, z, Vx, Vy, Vz, TFX, TFY, TFZ, D,B, dt, kT); % Positon update setup
end
%% Write inital file
bd_write(c_out,b_out,e_out,g_out,np,x,y,z)
fprintf('\n Production Step \n');


%% reupdate hydrodynamic tensor 
D = bd_RPY_hyd_v1(np,n3,a,L,mu,x,y,z);
D = D/(6*pi*mu*a);
[B,~] = chol(kT*D,'lower');
...
%% ADD flow
    for i = 1:(20.*n_prod)
     [Vx, Vy, Vz] = bd_fluid_v2(x, y, z, g); % calculate flow velocity
     [~, TFX, TFY, TFZ, ~] = bd_force_free_ID(np,n3,u,a,K,L,x,y,z,mu);

     if mod(i,period) == 0 %update hydrodynamics 
          D = bd_RPY_hyd_v1(np,n3,a,L,mu,x,y,z);
          D = D/(6*pi*mu*a);
         [B,~] = chol(kT*D,'lower');
     end
       
      [x, y, z] = bd_move_rpy_v2(n3, x, y, z, Vx, Vy, Vz, TFX, TFY, TFZ, D,B, dt, kT);% update position 
      
      if mod(i,w_step) == 0
         [x, y ,z] = bd_cent(x, y, z,h);
